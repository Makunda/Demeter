...
public class LevelsUtils {
    ...
    public static String getLevelLabelByNumber(int levelNumber) {
        switch (levelNumber) {
            ...
            default:
                throw new IllegalArgumentException(String.format("No level corresponds to number %d.", levelNumber));
        }
    }
    ...
    public static void refreshAllAbstractLevel(Neo4jAL neo4jAL, String applicationContext) throws Neo4jQueryException {
        ...
        for(int i = 4; i > 1; i--) {
            ...
            while (result.hasNext()) {
                ...
                String forgedToLevels = String.format("MATCH (level:%1$s:%2$s)-[:%3$s]->(:%4$s:%2$s)-->(o:%4$s:%2$s)<-[:%3$s]-(l:%1$s) WHERE ID(level)=%5$d RETURN DISTINCT l as level;",
                        labelN, applicationContext, IMAGING_AGGREGATES, labelN0, ln.getId());

                String forgedFromLevel = String.format("MATCH (level:%1$s:%2$s)-[:%3$s]->(:%4$s:%2$s)<--(o:%4$s:%2$s)<-[:%3$s]-(l:%1$s) WHERE ID(level)=%5$d RETURN DISTINCT l as level;",
                        labelN, applicationContext, IMAGING_AGGREGATES, labelN0, ln.getId());

                String deleteOldRelations = String.format("MATCH (level:%1$s:%2$s)-[r:%3$s]-(:%1s:%2$s) WHERE ID(level)=%4$d DELETE r",
     ...
     */
    public static Node refreshLevelLinks5(Neo4jAL neo4jAL, String applicationContext, Node nodeLevel) throws Neo4jQueryException {
     ...
     * @throws Neo4jQueryException
     */
    public static Node refreshLevelCount5(Neo4jAL neo4jAL, String applicationContext, Node levelNode) throws Neo4jQueryException {
