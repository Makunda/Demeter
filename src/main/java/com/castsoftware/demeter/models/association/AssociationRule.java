...
public class AssociationRule extends Neo4jObject {
    ...
    private static final String ERROR_PREFIX = "ARULx";

    // Label & statics properties
    private static final String LABEL = "AssociationRule";
    private static final String SOURCE_TYPE_PROPERTY = "SourceType";
    private static final String DEST_TYPE_PROPERTY = "SourceType";
    private static final String TO_CONFIG = "BelongsTo";
    ...
    public AssociationRule(Neo4jAL neo4jAL, Node node) throws Neo4jBadNodeFormatException {
        ...
        try {
            ...
            throw new Neo4jBadNodeFormatException(
                    "Failed to instanciate an association rule node", e, ERROR_PREFIX + "ARUL01");
        }
    }
    ...
    public Node createNode() throws Neo4jBadRequestException, Neo4jNoResult {
        String req =
                String.format(
                        "MERGE (n:`%1$s` { %2$s: $sourceType, %3$s: $destType }) RETURN n as node;",
                        getLabel(), getSourceTypeProperty(), getDestTypeProperty());
        Map<String, Object> params =
                Map.of(getSourceTypeProperty(), this.sourceType, getDestTypeProperty(), this.destType);
        ...
        try {
            ...
            if (!res.hasNext())
                throw new Neo4jNoResult(
                        "The creation request faield to instantiate the node", req, ERROR_PREFIX + "CREN01");

            return (Node) res.next().get("node");
        } catch (Neo4jQueryException e) {
            throw new Neo4jBadRequestException(
                    String.format("The request creating an association rule is not valid. Request: %s", req),
                    e,
                    ERROR_PREFIX + "CREN02");
        }
    }
