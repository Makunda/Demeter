...
public class ModuleNode extends Neo4jObject {
  ...
  private static final String IMAGING_LINKS_TO_OBJECTS =
      Configuration.get("imaging.node.module.links.to_objects");
  private static final String IMAGING_LINKS_TO_MODULES =
  ...
  public static ModuleNode fromNode(Neo4jAL neo4jAL, Node node) throws Neo4jBadNodeFormatException {
    ...
    try {
      ...
      throw new Neo4jBadNodeFormatException(
          getLabelProperty() + " instantiation from node.", ERROR_PREFIX + "FROMN2");
    }
  }
   ...
   * @throws Neo4jBadRequestException The Backup request contains forbidden argument that led to an
   *     error
   * @throws Neo4jNoResult Was not able to backup
   * @throws Neo4jQueryException The backup failed due to abad query
   */
  public Node createBackup(String applicationContext, List<Node> affectedNodes)
  ...
  public void deleteNode() throws Neo4jBadRequestException {
    String queryDomain =
        String.format("MATCH (p:%s) WHERE ID(p)=%d DETACH DELETE p;", LABEL, this.getNodeId());
    try {
      neo4jAL.executeQuery(queryDomain);
    } catch (Neo4jQueryException e) {
      throw new Neo4jBadRequestException(
          LABEL + " node deletion failed", queryDomain, e, ERROR_PREFIX + "DEL1");
    }
  }
}
