...
public abstract class Neo4jObject {
  ...
  /**
   * Parse the specified node property and extract the boolean value. During manipulations of the
   * property, some process can accidentally convert the value of the boolean to a text format. When
   * this happens, a classical boolean cast fails to execute and throw ClassCastException.
   *
  ...
  /**
   * Find an existing node in the DB matching the same properties. The node found will be assigned
   * to the object. (Warning : will return the first node encountered if two nodes have the exact
   * same properties)
   *
