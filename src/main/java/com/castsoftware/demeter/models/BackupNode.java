...
public class BackupNode extends Neo4jObject {
  ...
  private static final String NODE_GEN_REQUEST_PROPERTY =
      Configuration.get("demeter.backup.node.node_gen_request");
  private static final String NODE_LABEL_PROPERTY =
      Configuration.get("demeter.backup.node.node_label");

  private static final String OBJECT_LEVEL_PROPERTY =
      Configuration.get("imaging.node.object.level");
  private static final String LEVEL_NAME_PROPERTY = Configuration.get("imaging.node.level5.name");

  private static final String GEN_REQUEST_RETURN_VALUE =
      Configuration.get("demeter.backup.node.node_gen_request.return_val");

  private static final String BACKUP_COPY_RELATION =
      Configuration.get("demeter.backup.relationship.copy");
  private static final String BACKUP_NODES_RELATION =
      Configuration.get("demeter.backup.relationship.type");
  private static final String BACKUP_NODES_OLD_REL_NAME =
      Configuration.get("demeter.backup.relationship.old_relation_name");
   ...
   * @param backupRequest Associated backup request ( the request will be executed to recreate the
   *     node)
   * @param nodesAffected Potentials nodes affected by this change, to avoid a full backup.
   * @return The backup node created
   */
  public static Node createBackup(
      Neo4jAL neo4jAL,
      String applicationContext,
      Node node,
      String backupRequest,
      List<Node> nodesAffected)
    ...
    // Create a Copy Link Relationship to the  actual node. The copy link will be used if the node
    // wasn't deleted
    String mergeLink = String.format("MATCH (bk) WHERE ID(bk)=$idBk " +
                "WITH bk " +
                "MATCH (n) WHERE ID(n)=$idN " +
                "MERGE (bk)-[:%s]->(n)", copyRelName);
    Map<String, Object> params = Map.of("idBk", bkNode.getId(), "idN", node.getId());
    neo4jAL.executeQuery(mergeLink, params);
    ...
    for (Relationship rel : node.getRelationships()) {
      ...
      if (!affectedIds.contains(otherNode.getId()))
        continue; // Ignore node not part of potential affected candidates
      ...
      if (rel.getStartNodeId() == node.getId()) { // Outgoing rel
        mergeRel =
            String.format(
                "MATCH (n),(m) WHERE ID(n)=%d AND ID(m)=%d MERGE (n)-[r:%s]->(m) RETURN r as rel;",
                bkNode.getId(), otherNode.getId(), BACKUP_NODES_RELATION);
      } else { //  Incoming rel
        mergeRel =
            String.format(
                "MATCH (n),(m) WHERE ID(n)=%d AND ID(m)=%d MERGE (m)-[r:%s]->(n) RETURN r as rel;",
                bkNode.getId(), otherNode.getId(), BACKUP_NODES_RELATION);
      }
      ...
      if (!res.hasNext()) { // Is no result throw an error
        throw new Neo4jNoResult(
            "The request creating relationship produced no result.",
            mergeRel,
            ERROR_PREFIX + "CREA1");
      }
   ...
   */
  public static List<BackupNode> getApplicationBackupNode(
      Neo4jAL neo4jAL, String applicationContext, String nodeLabel)
      throws Neo4jBadRequestException, Neo4jBadNodeFormatException {
    ...
    try {
      ...
      throw new Neo4jBadRequestException(
          LABEL + " nodes retrieving by label failed", bkAppQuery, e, ERROR_PREFIX + "DEL1");
    }
  ...
  public Node createNode() throws Neo4jNoResult {
    ...
    try {
      ...
      String req = String.format("MERGE (o:`%s` { %s: $genReq, %s: $nodeLabel } ) RETURN o as node", LABEL,
              getNodeGenRequestProperty(), getNodeLabelProperty());
      Map<String, Object> params = Map.of("genReq", getNodeGenReq(), "nodeLabel", getNodeLabel());
      Result res = neo4jAL.executeQuery(req, params);

      if(!res.hasNext()) {
        throw new Neo4jNoResult("Failed to create a backup node.", req, ERROR_PREFIX + "CRN1" );
      }

      Node n = (Node) res.next().get("node");
      this.setNode(n);
      return n;
    } catch (NoSuchElementException | NullPointerException | Neo4jQueryException e) {
      throw new Neo4jNoResult(LABEL + "node creation failed", "", e, ERROR_PREFIX + "CRN2");
    }
  }

  /**
   * Generate the node associated with the request. If this node have backup parents node, they will
   * also be generated and linked to the newly created node. The backup node is consumed in the
   * operation
   *
   * @return The node created by this backup node
   */
  public Node startBackup()
    ...
    // Check if the backup node copies another node. If so, the node used will be the original one.
    // Otherwise the node is recreated from the backup request
    Node referenceNode = null;
    Iterator<Relationship> relIt =
        backupNode.getRelationships(Direction.OUTGOING, copyRelationship).iterator();
    if (relIt.hasNext()) {
      // Get existing node being copy by the backup
      referenceNode = relIt.next().getEndNode();
      neo4jAL.logInfo("The backup node copies an existing node. No recreation of Levels.");
    } else {
      // Get the node returned by the query
      try {
        // Generate the node via the nodeGenRequest
        String req = getNodeGenReq().replaceAll("\\\\'", "'");
        neo4jAL.logInfo("Backup request to be executed : " + req);
        Result res = neo4jAL.executeQuery(req);

        // Throw error if no result returned by the query
        if (!res.hasNext()) {
          throw new Neo4jNoResult(
              "The request supposed to generate the node produced no result.",
              getNodeGenReq(),
              ERROR_PREFIX + "GENB1");
        }
        ...
        throw new Neo4jBadRequestException(
            "The request supposed to generate does not return result labeled correctly'.",
            ERROR_PREFIX + "GENB2");
      }
    }

    // Get attached relationships and recreate the relationships
    for (Relationship relation :
        backupNode.getRelationships(RelationshipType.withName(BACKUP_NODES_RELATION))) {
      ...
      try {
        ...
        String message =
            String.format(
                "The relationship with label '%s' between nodes with id %d and %d is malformed. Missing name value",
                relation.getType().name(), backupNode.getId(), referenceNode.getId());
        ...
        mergeRel =
            String.format(
                "MATCH (n) WHERE ID(n)=%d " +
                        "WITH n " +
                        "MATCH (m) WHERE ID(m)=%d " +
                        "MERGE (n)-[r:%s]->(m) RETURN r as rel;",
                referenceNode.getId(), otherNode.getId(), toCreateRelName);
      } else { // Merge incoming
        otherNode = relation.getStartNode();
        mergeRel =
            String.format(
                "MATCH (n),(m) WHERE ID(n)=%d AND ID(m)=%d MERGE (n)-[r:%s]->(m) RETURN r as rel;",
                otherNode.getId(), referenceNode.getId(), toCreateRelName);
      }
      ...
      if (otherNode.hasProperty(OBJECT_LEVEL_PROPERTY)) {
        otherNode.setProperty(
            OBJECT_LEVEL_PROPERTY, (String) referenceNode.getProperty(LEVEL_NAME_PROPERTY));
      }
