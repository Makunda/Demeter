...
public class BackupController {
    ...
    public static List<Node> undoLevelGroups(Neo4jAL neo4jAL, String applicationContext)
        ...
        List<String> toDeleteLevelsNAme = new ArrayList<>();
        ...
        for (Level5Node level : Level5Node.getAllNodesByApplication(neo4jAL, applicationContext)) {
            ...
            if (fullName.matches(".*##(" + GENERATED_LEVEL_PREFIX + ".*)")) {
                ...
                toDeleteLevelsNAme.add(level.getName());
                level.deleteNode();
            } else {
        ...
        String forgedLabel;
        String forgeRequest;
        for (String levelName : toDeleteLevelsNAme) {

            // Find nodes with the ancient level 5 name
            forgeRequest =
                    String.format(
                            "MATCH (o:Object:`%1$s`)<-[:%2$s]-(l:`%3$s`) WHERE o.Level='%4$s' AND NOT l.Name='%4$s' SET o.Level=l.Name;",
                            applicationContext, IMAGING_AGGREGATES, IMAGING_LEVEL_5_LABEL, levelName);
