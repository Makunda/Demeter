...
public class DocumentController {
    ...
    private static final String USECASE_TO_DOC_REL =
            Configuration.get("neo4j.relationships.use_case.to_document");

    /**
     * Add a DocumentNode and attach it to the specified useCase node. On its execution, the node will
     * create a Document It and link it to every node matching it.
     *
     ...
     * @param description         Description of the Request, used to quickly understand the action of the
     *                            request
     ...
     */
    public static Node addDocumentNode(
            Neo4jAL nal,
            String title,
            String request,
            Boolean active,
        ...
        if (!parent.hasLabel(useCaseLabel)) {
            throw new Neo4jBadRequestException(
                    String.format(
                            ...
                            DocumentNode.getLabel(), UseCaseNode.getLabel()),
                    ERROR_PREFIX + "ADDU1");
        }

        DocumentNode docNode =
                new DocumentNode(nal, title, request, active, description, documentDescription);
     ...
     * @throws Neo4jNoResult
     */
    public static List<DocumentNode> getSelectedDocuments(Neo4jAL neo4jAL, String configurationName)
            throws Neo4jBadRequestException, Neo4jQueryException, Neo4jNoResult {

        Label documentLabel = Label.label(DocumentNode.getLabel());
        Set<Node> documents =
                UseCaseController.searchByLabelInActiveBranches(neo4jAL, configurationName, documentLabel);
