...
public class AggregationController {
    ...
    private List<Long> verifyNodes(List<Long> idNodes) throws Neo4jQueryException {
        String req =
                String.format(
                        "MATCH (l:%s:`%s`) WHERE ID(l)=$id RETURN l as node", CUSTOM_LABEL, application);
     ...
     */
    private void mergeAggregationNode(Long idAggregationNode, List<Long> idNodes)
            throws Neo4jQueryException {
        String req =
                String.format(
                        "MATCH (a:%1$s:`%2$s`) WHERE ID(a)=$id "
                                + "WITH a LIMIT 1 "
                                + "MATCH (l:%3$s:`%2$s`) WHERE ID(l) IN $idList "
                                + "MERGE (a)-[:HAS]->(l)",
                        AGGREGATION_LABEL, application, CUSTOM_LABEL);
     ...
     */
    public Optional<Node> findAggregationNodeByName(String aggregationName)
            throws Neo4jQueryException {
        String req =
                String.format(
                        "MATCH (a:%s:`%s`) WHERE a.Name=$name RETURN a as node",
                        AGGREGATION_LABEL, application);
     ...
     */
    private Node createAggregationNode(String aggregationName)
        ...
        if (!res.hasNext()) {
            this.neo4jAL.logError(
                    "Failed to create a custom aggregation. The request returned no results");
     ...
     */
    public Node createCustom(Long aggregationId, String customName, List<Long> idNodes)
            throws Neo4jQueryException, Neo4jNoResult {
        ...
        if (aggregationNode.isEmpty()) { // If the aggregation is not found throw an error
            this.neo4jAL.logError(
                    String.format(
                            "The aggregation node with id '%d' " + "doesn't exist in the application '%s'",
                            aggregationId, application));
        ...
        String req =
                String.format(
                        "MATCH (o:`%s`:Object) WHERE ID(o) IN $idNode RETURN ID(o) as idNode", application);
        ...
        this.neo4jAL.logInfo(
                String.format("Creating a custom nodes with %d objects attached.", nodeIdList.size()));
    ...
    public Optional<Node> findAggregationNodeById(Long aggregationId) throws Neo4jQueryException {
        String req =
                String.format(
                        "MATCH (a:%s:`%s`) WHERE ID(a)=$id RETURN a as node", AGGREGATION_LABEL, application);
     ...
     */
    private Optional<Node> findCustomNode(Long aggregationId, String customName)
            throws Neo4jQueryException {
        String reqLinks =
                String.format(
                        "MATCH (a:%s:`%2$s`) WHERE ID(a)=$id "
                                + "WITH a "
     ...
     */
    private Node createCustomNode(String name, List<Long> idNodes)
        ...
        String reqCustom =
                String.format(
                        "CREATE (c:%s:`%s`) "
                                ...
                                + "SET c.FullName=$name "
                                + "RETURN c as node",
                        CUSTOM_LABEL, application);
        Map<String, Object> customParams =
                Map.of("name", name, "level", 1, "count", idNodes.size(), "fullName", name);
        ...
        if (!resCustom.hasNext()) {
            ...
            throw new Neo4jNoResult(
                    "Failed to create a custom node. The request return no results.",
                    reqCustom,
                    "AGGCxCREAC01");
        }
     ...
     */
    private void mergeCustomNodeRelationships(Node customNode, List<Long> idNodes)
            throws Neo4jQueryException {
        // Link the objects
        String reqLinks =
                String.format(
        ...
        neo4jAL.logInfo(
                String.format(
                        "Executed %d queries to link the " + "objects for custom node with id '%d' ",
                        count, customNode.getId()));
    }
    ...
    private void linkCustomToAggregate(Long aggregationId, Long customId) throws Neo4jQueryException {
        try {
            String req =
                    String.format(
                            "MATCH (a:%1$s:`%2$s`) WHERE ID(a)=$aggregationId "
                                    + "WITH a "
                                    + "MATCH (c:%3$s:`%2$s`) WHERE ID(c)=$customId "
                                    + "MERGE (a)-[:HAS]->(c)",
                            AGGREGATION_LABEL, application, CUSTOM_LABEL);
            Map<String, Object> params = Map.of("aggregationId", aggregationId, "customId", customId);
    ...
    public void refreshAggregation(Long idAggregation) throws Neo4jQueryException {
        ...
        this.neo4jAL.logInfo(
                String.format("The aggregation with id '%d' has been refreshed.", idAggregation));
    }
    ...
    public List<Node> getCustomNodesAttached(Long idAggregation) throws Neo4jQueryException {
        String req =
                String.format(
                        "MATCH (a:%1$s:`%2$s`)-[]->(c:%3$s) " + "WHERE ID(a)=$id " + "RETURN  c as node",
                        AGGREGATION_LABEL, application, CUSTOM_LABEL);
    ...
    private void getLinksNumber(Long custom1, Long custom2) throws Neo4jQueryException {
        String refreshLinks =
                String.format(
                        "MATCH (l:%2$s:`%1$s`)-[]->(o)-->(o2)<-[]-(l2:%2$s:`%1$s`) "
                                + "WHERE ID(l)=$id1 and ID(l2)=$id2 "
                                + "AND ID(l)<>ID(l2) "
                                + "WITH l, l2, o LIMIT 1 "
                                + "MERGE (l)-[:References]->(l2); ",
                        application, CUSTOM_LABEL);
    ...
    public void deleteAggregationByName(String aggregationName) throws Neo4jQueryException {
        ...
        if (aggNode.isPresent()) {
            this.neo4jAL.logInfo(
                    String.format(
                            "An aggregation node with name '%s' in application '%s' "
                                    + "has been found and will now be deleted.",
            ...
            this.neo4jAL.logError(
                    String.format(
                            "No aggregation node with name '%s' was found in application '%s'.",
                            aggregationName, application));
        }
    }
}
