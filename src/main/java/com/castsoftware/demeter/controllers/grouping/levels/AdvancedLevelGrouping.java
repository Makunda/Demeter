...
public class AdvancedLevelGrouping {
	 ...
	 */
	private Optional<Node> findLevelByNameAndDepth(String application, String name, int depth) throws Neo4jQueryException {
		String req = String.format("MATCH (level:Level%2$d:`%1$s`) WHERE level.Name=$name " +
						"RETURN level as node LIMIT 1", application, depth);
	 ...
	 * @return
	 */
	private Node createNamedLevel(String application, String name, String fullName, int depth) throws Neo4jQueryException, Neo4jNoResult {
		String req = String.format(" MERGE (level:Level%2$d:`%1$s` { "+
					"Concept: false, " +
				    "Color: 'rgb(176,196,222)', " +
							...
							"Name: $name" +
				" }) RETURN level as node;", application, depth);
		Result results = neo4jAL.executeQuery(req, Map.of("name", name, "fullName", fullName, "depth", depth));
		if(results.hasNext()) {
			return (Node) results.next().get("node");
		}
		else throw new Neo4jNoResult("Failed to create a named level the request returned no results.", req, ERROR_PREFIX+"CREAN01");
	}
	 ...
	 */
	private void linkLevelToChild(String application, Node parent, Node children) throws Neo4jQueryException {
		// Delete old parent relationships
		String deleteParentRel = "MATCH (child:`%1$s`)<-[r:Aggregates]-() WHERE ID(child)=$childId " +
				"DELETE r";
		neo4jAL.executeQuery(deleteParentRel, Map.of( "childId", children.getId()));

		// Merge relationships
		String merge = String.format("MATCH (parent:`%1$s`) WHERE ID(parent)=$parentId WITH parent " +
						"MATCH (child:`%1$s`) WHERE ID(child)=$childId " +
						"MERGE (parent)-[:Aggregates]->(child)", application);
	 ...
	 */
	private void reassignObjects(String application, Node level5, List<Long> idList) throws Neo4jQueryException {
		String req = String.format("MATCH (level:Level5:`%1$s`) WHERE ID(level)=$levelId " +
						"WITH level " +
						...
						"RETURN DISTINCT o as node", application);
	 ...
	 */
	public List<Node> groupWithCategory(String application, String category, String name, List<Long> idList) throws Neo4jQueryException, Neo4jNoResult {
	 ...
	 * @return The levels created
	 */
	public List<Node> groupWithTaxonomy(String application, String level1, String level2, String level3,
										String level4, String level5, List<Long> idList)
		...
		return List.of(level1Node.get(), level2Node.get(), level3Node.get(), level4Node.get(), level5Node.get());
	}
