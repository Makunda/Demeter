...
public class ExportController {
	...
	public void exportState(String application, String path, String packageFile) throws Exception {
		...
		if( !Files.exists(directoryPath)) throw new Exception(String.format("The specified directory doesn't exist. Path : '%s'.", path));

		// Check if file exists
		String fileName = packageFile + ".bak_" + part;
		Path filePath = Path.of(path, fileName);
		if( Files.exists(filePath)) throw new Exception(String.format("A file named '%s' already exists at path : '%s'.", packageFile, path));
