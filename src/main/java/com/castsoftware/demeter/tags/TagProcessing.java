...
public class TagProcessing {
  ...
  public static final String ANCHOR_COUNT_RETURN_VALUE =
      Configuration.get("tag.anchors.countReturn.label");
    ...
    if (m.find()) {
      ...
      if (m.groupCount() < 1)
        throw new Neo4JTemplateLanguageException(
            "Invalid return tag usage.", request, "TAGPxPRRT01");
  ...
  public static String forgeCountRequest(String request) throws Neo4JTemplateLanguageException {
    ...
    if (m.find()) {
      ...
      if (m.groupCount() < 1)
        throw new Neo4JTemplateLanguageException(
            "Invalid count tag usage.", request, "TAGPxFCOR01");
